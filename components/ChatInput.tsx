...
import { toastArray } from "../utils/commonFunctions";
...
const ChatInput = ({ chatId, query }: { chatId: string; query?: string }) => {
  ...
  const sendMessage = async () => {
    ...
    if (!!toastArray[loadingIndex]) {
      notification = toast.loading(toastArray[loadingIndex].loading);
    } else {
      notification = toast.loading("Getting the magic broom...");
    }
    ...
    await fetch("/api/askQuestion", {
      ...
      if (!!toastArray[loadingIndex]) {
        toast.success(toastArray[loadingIndex].loaded, { id: notification });
      } else {
